...
func TestSendCustomAppMetrics_ReadOnlyMode(t *testing.T) {
	// Happy path only - read-only mode
	...
	pactInteraction := func() {
		pact.
			AddInteraction().
			Given("Send valid custom app metrics in read-only mode").
			UponReceiving("A request to send custom app metrics in read-only mode").
			WithRequest(dsl.Request{
				Method: http.MethodPost,
				Headers: dsl.MapMatcher{
					"User-Agent":                             dsl.String("Replicated-SDK/v0.0.0-unknown"),
	...
	t.Run("Send valid custom app metrics in read-only mode", func(t *testing.T) {
		pactInteraction()
