package license

import (
	"testing"

	licensewrappertypes "github.com/replicatedhq/kotskinds/pkg/licensewrapper/types"
	"github.com/stretchr/testify/require"
)

func TestVerifySignature(t *testing.T) {
	tests := []struct {
		name             string
		license          string
		wantErr          bool
		wantValidLicense bool
		wantErrMsg       string
	}{
		{
			name: "basic valid signature",
			license: `apiVersion: kots.io/v1beta1
kind: License
metadata:
  name: testcustomer
spec:
  appSlug: my-app
  channelID: 1vusIYZLAVxMG6q760OJmRKj5i5
  channelName: My Channel
  customerName: Test Customer
  endpoint: https://replicated.app
  entitlements:
    bool_field:
      title: Bool Field
      value: true
      valueType: Boolean
    expires_at:
      description: License Expiration
      title: Expiration
      value: "2030-07-27T00:00:00Z"
      valueType: String
    hidden_field:
      isHidden: true
      title: Hidden Field
      value: this is secret
      valueType: String
    int_field:
      title: Int Field
      value: 123
      valueType: Integer
    string_field:
      title: StringField
      value: single line text
      valueType: String
    text_field:
      title: Text Field
      value: |-
        multi
        line
        text
      valueType: Text
  isAirgapSupported: true
  isGitOpsSupported: true
  isSnapshotSupported: true
  licenseID: 1vusOokxAVp1tkRGuyxnF23PJcq
  licenseSequence: 7
  licenseType: prod
  signature: 
`,
			wantErr:          false,
			wantValidLicense: true,
			wantErrMsg:       "",
		},
		{
			name: "invalid signature",
			license: `apiVersion: kots.io/v1beta1
kind: License
metadata:
  name: testcustomer
spec:
  appSlug: my-app
  channelID: 1vusIYZLAVxMG6q760OJmRKj5i5
  channelName: My Channel
  customerName: Test Customer
  endpoint: https://replicated.app
  entitlements:
    bool_field:
      title: Bool Field
      value: true
      valueType: Boolean
    expires_at:
      description: License Expiration
      title: Expiration
      value: "2030-07-27T00:00:00Z"
      valueType: String
    hidden_field:
      isHidden: true
      title: Hidden Field
      value: this is secret
      valueType: String
    int_field:
      title: Int Field
      value: 123
      valueType: Integer
    string_field:
      title: StringField
      value: single line text
      valueType: String
    text_field:
      title: Text Field
      value: |-
        multi
        line
        text
      valueType: Text
  isAirgapSupported: true
  isGitOpsSupported: true
  isSnapshotSupported: true
  licenseID: 1vusOokxAVp1tkRGuyxnF23PJcq
  licenseSequence: 7
  licenseType: prod
  signature: 
`,
			wantErr:          true,
			wantValidLicense: false,
			wantErrMsg:       "v1 license signature verification failed: signature verification failed: crypto/rsa: verification error",
		},
		{
			name: "licenseID field changed",
			license: `apiVersion: kots.io/v1beta1
kind: License
metadata:
  name: testcustomer
spec:
  appSlug: my-app
  channelID: 1vusIYZLAVxMG6q760OJmRKj5i5
  channelName: My Channel
  customerName: Test Customer
  endpoint: https://replicated.app
  entitlements:
    bool_field:
      title: Bool Field
      value: true
      valueType: Boolean
    expires_at:
      description: License Expiration
      title: Expiration
      value: "2030-07-27T00:00:00Z"
      valueType: String
    hidden_field:
      isHidden: true
      title: Hidden Field
      value: this is secret
      valueType: String
    int_field:
      title: Int Field
      value: 123
      valueType: Integer
    string_field:
      title: StringField
      value: single line text
      valueType: String
    text_field:
      title: Text Field
      value: |-
        multi
        line
        text
      valueType: Text
  isAirgapSupported: true
  isGitOpsSupported: true
  isSnapshotSupported: true
  licenseID: 1vusOokxAVp1tkRGuyxnF23PJcq-modified
  licenseSequence: 7
  licenseType: prod
  signature: 
`,
			wantErr:          true,
			wantValidLicense: true,
			wantErrMsg:       `license data validation failed: licenseID field has changed to "1vusOokxAVp1tkRGuyxnF23PJcq-modified" (license) from "1vusOokxAVp1tkRGuyxnF23PJcq" (within signature)`,
		},
		{
			name: "endpoint field changed",
			license: `apiVersion: kots.io/v1beta1
kind: License
metadata:
  name: testcustomer
spec:
  appSlug: my-app
  channelID: 1vusIYZLAVxMG6q760OJmRKj5i5
  channelName: My Channel
  customerName: Test Customer
  endpoint: https://replicated.app.modified
  entitlements:
    bool_field:
      title: Bool Field
      value: true
      valueType: Boolean
    expires_at:
      description: License Expiration
      title: Expiration
      value: "2030-07-27T00:00:00Z"
      valueType: String
    hidden_field:
      isHidden: true
      title: Hidden Field
      value: this is secret
      valueType: String
    int_field:
      title: Int Field
      value: 123
      valueType: Integer
    string_field:
      title: StringField
      value: single line text
      valueType: String
    text_field:
      title: Text Field
      value: |-
        multi
        line
        text
      valueType: Text
  isAirgapSupported: true
  isGitOpsSupported: true
  isSnapshotSupported: true
  licenseID: 1vusOokxAVp1tkRGuyxnF23PJcq
  licenseSequence: 7
  licenseType: prod
  signature: 
`,
			wantErr:          true,
			wantValidLicense: true,
			wantErrMsg:       `license data validation failed: endpoint field has changed to "https://replicated.app.modified" (license) from "https://replicated.app" (within signature)`,
		},
		{
			name: "channel ID removed from signature",
			license: `apiVersion: kots.io/v1beta1
kind: License
metadata:
  name: testcustomer
spec:
  appSlug: my-app
  channelName: My Channel
  customerName: Test Customer
  endpoint: https://replicated.app
  entitlements:
    bool_field:
      title: Bool Field
      value: true
      valueType: Boolean
    expires_at:
      description: License Expiration
      title: Expiration
      value: "2030-07-27T00:00:00Z"
      valueType: String
    hidden_field:
      isHidden: true
      title: Hidden Field
      value: this is secret
      valueType: String
    int_field:
      title: Int Field
      value: 123
      valueType: Integer
    string_field:
      title: StringField
      value: single line text
      valueType: String
    text_field:
      title: Text Field
      value: |-
        multi
        line
        text
      valueType: Text
  isAirgapSupported: true
  isGitOpsSupported: true
  isSnapshotSupported: true
  licenseID: 1vusOokxAVp1tkRGuyxnF23PJcq
  licenseSequence: 7
  licenseType: prod
  signature: 
`,
			wantErr:          true,
			wantValidLicense: true,
			wantErrMsg:       `license data validation failed: channelID field has changed to "" (license) from "1vusIYZLAVxMG6q760OJmRKj5i5" (within signature)`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := require.New(t)

			license, err := LoadLicenseFromBytes([]byte(tt.license))
			req.NoError(err)

			err = license.VerifySignature()

			if tt.wantErr {
				req.Error(err)
				req.Equal(tt.wantErrMsg, err.Error())
				if tt.wantValidLicense {
					req.True(licensewrappertypes.IsLicenseDataValidationError(err), "error must be a license data validation error")
				} else {
					req.False(licensewrappertypes.IsLicenseDataValidationError(err), "error must NOT be a license data validation error")
				}
			} else {
				req.NoError(err)
			}

			if tt.wantValidLicense {
				// ensure that the license data matches expectations after signature verification, despite changes in several test cases
				req.Equal("1vusOokxAVp1tkRGuyxnF23PJcq", license.GetLicenseID())

				req.Equal("https://replicated.app", license.GetEndpoint())

				req.Equal("1vusIYZLAVxMG6q760OJmRKj5i5", license.GetChannelID())
			}
		})
	}
}
